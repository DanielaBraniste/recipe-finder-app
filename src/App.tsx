...
const App: React.FC = () => {
  ...
  const submitToNetlify = (action: string, recipes: Recipe[]) => {
    const formData = {
      ...
      'action': action,
      'continent': selectedCuisine,
      'recipes-found': recipes.map(r => r.name).join(', '),
      'recipe-count': recipes.length.toString()
    };
  ...
  const submitMealPlanToNetlify = (recipes: Recipe[]) => {
    const formData = {
      ...
      'continents-selected': selectedContinents.join(', '),
      'dietary-preference': dietaryPreference,
      'recipes-generated': recipes.map(r => `${r.name} (${r.cuisine})`).join(', '),
      'recipe-count': recipes.length.toString()
    };
    ...
    fetch('/', {
      ...
      .then(() => console.log('Meal planner form submitted successfully'))
      .catch(error => console.error('Meal planner form submission error:', error));
  };
  ...
  const continentOptions = [
    ...
    { display: 'Middle Eastern', value: 'Middle East' },
  ...
  const commonIngredients = [
    ...
    'pork', 'bacon', 'pork chops', 'fish', 'salmon', 'tuna', 'cod',
    'shrimp', 'prawns', 'rice', 'white rice', 'brown rice', 'pasta', 'spaghetti',
    'noodles', 'bread', 'potatoes', 'sweet potatoes', 'onions', 'garlic', 'tomatoes',
    'carrots', 'broccoli', 'spinach', 'cheese', 'mozzarella cheese', 'parmesan cheese',
    'eggs', 'milk', 'butter', 'olive oil', 'salt', 'pepper', 'basil', 'oregano'
  ];

  // Handle ingredient input changes with autocomplete
  ...
  const copyShoppingListToClipboard = async () => {
    ...
    Object.entries(shoppingList)
      ...
      .forEach(([category, items]) => {
        shoppingListText += `📦 ${category.toUpperCase()}\n`;
        shoppingListText += '───────────────────────────────────\n';
        items.forEach(item => {
          shoppingListText += `☐ ${item.name} - ${item.quantity} ${item.unit}\n`;
        });
        shoppingListText += '\n';
      });
    
    shoppingListText += '═══════════════════════════════════\n';
    shoppingListText += `Generated by Kitchen Planner\n`;
    shoppingListText += `${days.length} days of delicious meals! 🍽️`;
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-2 bg-white border-2 border-orange-500 rounded-xl shadow-2xl max-h-60 overflow-y-auto z-50">
                ...
                <div className="flex-1">
                  ...
                  >
                    ...
                    {continentOptions.map(continent => (
                      <option key={continent.value} value={continent.value}>{continent.display}</option>
                    ))}
                  </select>
                </div>
              ...
              {searchResults.length === 0 ? (
                <div className="bg-white rounded-xl p-8 shadow-lg border border-gray-200 text-center">
                  <p className="text-gray-600 text-lg">
                    ...
                    Try adding more common ingredients or selecting "All Continents" to see more options.
                  </p>
                </div>
              ) : (
